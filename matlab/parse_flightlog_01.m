function status = parse_flightlog_01(varargin)
...
function parse_data_line(theline)
  ...
  global block_len;
	...
	%DATA[0, 0, [0,0,0], [0.000000, 0.000000, 0.000000], [0.000000, 0.000000, 0.000000], [0.000000, 0.000000, 0.000000], [0.000000, 0.000000, 0.000000], [0, 0, 0]]
...
% Expand the arrays if we're out of space 
  ...
  if flight_log_data.idx > length(flight_log_data.rover_t)
	...
	flight_log_data.rover_orient = [flight_log_data.rover_ex_gyro; zeros(block_len, 3)];	
	flight_log_data.rover_orient = [flight_log_data.rover_ex_acc; zeros(block_len, 3)];	
	flight_log_data.rover_orient = [flight_log_data.rover_int_acc; zeros(block_len, 3)];	
	flight_log_data.rover_orient = [flight_log_data.rover_ex_mag; zeros(block_len, 3)];	
	flight_log_data.rover_orient = [flight_log_data.rover_int_mag; zeros(block_len, 3)];	
  end

% Parse the string into the struct
  [flight_log_data.rover_t(flight_log_data.idx), ...
   flight_log_data.rover_status(flight_log_data.idx), ...
   ...
   flight_log_data.rover_int_mag(flight_log_data.idx, 3)] = ...
    strread(theline, 'DATA[%u %u [%u%u%u] [%n %n %n] [%n %n %n] [%n %n %n] [%n %n %n] [%n %n %n]]', 'delimiter', ',');
% end
